...
def send_welcome(message):
    ...
    log_interacted_user(message.chat.id)
    ...
    #     send_unsubs_button = types.KeyboardButton("📨 Отправить рассылку для взаимодействовавших")
    #     # markup.add(send_button, send_unsub_button, send_unsubs_button)
    #     markup.add(send_button)
    #     markup.add(send_unsub_button)
    #     markup.add(send_unsubs_button)
...
def send_message_to_interacted_users(message, db_paths):
    ...
    users = get_interacted_users(db_paths)
...
# Пример вызова этой функции администратором
@bot.message_handler(func=lambda message: message.text == "📨 Отправить рассылку для взаимодействовавших")
def handle_send_to_interacted_users(message):
    if str(message.chat.id) == ADMIN_ID:  # Проверка, что это администратор
        bot.send_message(message.chat.id, "Введите текст сообщения для рассылки пользователям, которые взаимодействовали с ботом:")
        bot.register_next_step_handler(message, lambda msg: process_interacted_users_message(msg))

def process_interacted_users_message(message):
    admin_message = message.text
    db_paths = "C:\\Users\\anm24\\Desktop\\test\\interacted_users.db"  # Путь к базе данных

    send_message_to_interacted_users(admin_message, db_paths)
    bot.send_message(message.chat.id, "Сообщение отправлено всем пользователям, которые взаимодействовали с ботом.")
...
🇰🇿 Kaspi (для оплаты в тенге): 4400 4302 9567 5877 \n(получатель: Татьяна С)
...
def handle_receipt(message):
    ...
    # Проверяем, если присланный документ является PDF
    if message.content_type == 'document' and message.document.mime_type == 'application/pdf':
        # Пересылаем PDF чек админу с информацией о пользователе
        bot.send_message(user_id, "📩 PDF чек отправлен админу для подтверждения.")
        bot.forward_message(ADMIN_ID, user_id, message.message_id)
    elif message.content_type == 'photo':
        # Пересылаем фото чека админу с информацией о пользователе
        bot.send_message(user_id, "📩 Чек (фото) отправлен админу для подтверждения.")
        bot.forward_message(ADMIN_ID, user_id, message.message_id)
...
# def create_one_time_invite_link():
    ...
    #     )
    #     print(CHANNEL_ID)
    #     print(f"Созданная ссылка: {invite_link}")
    #     return invite_link.invite_link
    # except Exception as e:
    #     bot.send_message(ADMIN_ID, f"Ошибка при создании ссылки для канала: {e}")
    #     return None
    # # try:
    #     # Устанавливаем срок действия ссылки на 24 часа
    #     expire_time = datetime.now() + timedelta(hours=24)
    #     expire_timestamp = int(expire_time.timestamp())

    #     # Создаем ссылку с ограничением по времени (24 часа) и без ограничений по количеству участников
    #     invite_link = bot.create_chat_invite_link(
    #         chat_id=CHANNEL_ID,
    #         expire_date=expire_timestamp,  # Срок действия ссылки в формате timestamp
    #         member_limit=None              # Без ограничений по количеству участников
    #     )
    #     print(f"Созданная ссылка с ограничением по времени: {invite_link.invite_link}")
    #     return invite_link.invite_link
    # except Exception as e:
    #     bot.send_message(ADMIN_ID, f"Ошибка при создании ссылки с временным ограничением: {e}")
    #     return None
...
def get_channel_id(message):
    if str(message.chat.id) == ADMIN_ID:
        ...
        invite_link = 'https://t.me/+NIwI2QUybk9mNDgy'  # ваша ссылка приглашения
        try:
